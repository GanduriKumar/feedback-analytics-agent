...
print(f"Found {len(filtered_reviews)} new reviews (skipped {len(cleaned_reviews) - len(filtered_reviews)} existing)")

# Early exit if no new reviews to process
if not filtered_reviews:
    print("No new reviews to process. Exiting without embedding.")
    exit(0)
...
try:
    for i in range(0, len(filtered_reviews), BATCH_SIZE):
        ...
        print(f"  Batch {i//BATCH_SIZE + 1}: Processed {progress}/{len(filtered_reviews)} ({percentage:.1f}%) in {batch_time:.2f}s")
...
try:
    ...
    for i in range(0, len(filtered_reviews), UPSERT_BATCH_SIZE):
        ...
        print(f"  Batch {i//UPSERT_BATCH_SIZE + 1}: Upserted {end_idx}/{len(filtered_reviews)} ({percentage:.1f}%) in {batch_time:.2f}s")
...
print(f"Total reviews processed this run: {len(filtered_reviews)} (new)")
